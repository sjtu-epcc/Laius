/*
 * File: /home/haohao/Projects/Paper/reference/mps/src/cudnn/cudnn_all_server.cc
 * Project: /home/haohao/Projects/Paper/reference/mps
 * Created Date: Monday, December 10th 2018, 6:28:34 pm
 * Author: Raphael-Hao
 * -----
 * Last Modified: Saturday, January 5th 2019, 9:15:54 am
...
int main(int argc, char const *argv[])
{
    ...
    CHECK_NE(fd, -1);
    set_args.shm_set = (ShmSet *)mmap(NULL, SHM_SET_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    int ret = close(fd);
    CHECK_NE(ret, -1);
    // map shared memory for compute
    fd = shm_open(getenv("SHARED_COMPUTE_FNAME"), O_RDWR, S_IRUSR | S_IWUSR);
    CHECK_NE(fd, -1);
    compute_args.shm_compute = (ShmCompute *)mmap(NULL, SHM_COMPUTE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ret = close(fd);
    CHECK_NE(ret, -1);
