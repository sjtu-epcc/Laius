...
#include <map>
#include "/home/laius/laius/Laius/mps_cudnn/include/optimized/cudnn_ipc.hpp"
#include "ipc.hpp"
using namespace std;
...
int main()
{
	...
	int counter = 0;
	timeval start_api,endapi,tmpapi;
	while (true)
	{
		...
		if(cudnn_api->process_flag == 0)
		{
			...
			if(counter% ISSYNC == 1)
			{
				...
				{
					//......
                    /*int tmp_ratio = cudnn_api->GPU_ratio;
                    for (i= cudnn_api->GPU_ratio;i<=10;i++)
                    {
                        cudnn_api->GPU_ratio++;
                        if((table[i].running_time - table[tmp_ratio].running_time) -
                            (cudnn_api->running_time[i] - cudnn_api->running_time[tmp_ratio]) >= timeuse - cudnn_api->time_predict)
                            break;
                    }*/

					isLate = 0;
				}
//				GPU_resource -= cudnn_api->GPU_ratio;
//				max_bandwidth -= cudnn_api->sch.band_predict;
//				gettimeofday(&startapi,0);
		...
		{
			printf("the process_flag is %d and changing to 3\n",cudnn_api->process_flag);
//			GPU_resource += cudnn_api->GPU_ratio;
//			max_bandwidth += cudnn_api->sch.band_predict;
			cudnn_api->process_flag = 3;
//			gettimeofday(&endapi,0);
//			double timeuse = 1000000*(endapi.tv_sec-startapi.tv_sec) + endapi.tv_usec - startapi.tv_usec;
//			timeuse /= 1000;
//			if(timeuse >cudnn_api->sch.time_predict)
//				isLate = 1;
		}
