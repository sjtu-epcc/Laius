...
int main()
{
    ...
    while (true)
    {
        ...
        if(sem_trywait(&cudnn_api->sync.sch_sem)==0)
        {
           ...
           // time_predict += cudnn_api->cudnn_call.duration[cudnn_api->self_percent / 10];
            time_predict += cudnn_api->cudnn_call.duration[20 / 10];
           /* for (int i=1;i<=10;i++)
            {
            ...
            if (counter % ISSYNC == 0)
            {
                sem_post(&cudnn_api->sync.if_sync);
            }
...
//                    printf("using %d percentage\n",cudnn_api->percent_flag);
                ...
                {
                    cudnn_api->percent_flag = cudnn_api->self_percent;
                    if(change == 1)
                    {
                        change = 0;
//                        printf("roll back to self_percent: %d\n",cudnn_api->self_percent);
                    }

                }
                GPU_resource -= cudnn_api->percent_flag / 10;
                ss.push(cudnn_api->percent_flag /10);
                printf("get %d and GPU ratio is %d\n",cudnn_api->percent_flag / 10,GPU_resource);
           ...
           // cudnn_api->process_flag = 1;
            if (cudnn_api->self_percent == cudnn_api->percent_flag)
            {
               // printf("cc\n");
                //sem_post(&cudnn_api->sync.if_sync);
                sem_post(&cudnn_api->sync.percent_sem[0]);
                pthread_barrier_wait(&cudnn_api->sync.barrier); //不切换
                //sem_wait(&cudnn_api->sync.synced);
        ...
        if (sem_trywait(&cudnn_api->sync.synced) == 0)
        {
            while(ss.size()>0)
            {
                GPU_resource += ss.top();
        ...
        // GPU_resource += cudnn_api->percent_flag / 10;
            printf("put %d and GPU_ratio is %d\n",cudnn_api->percent_flag /10,GPU_resource);
            max_bandwidth += sum_band;
                //sem_post(&cudnn_api->sync.sch_sem);
            gettimeofday(&endapi, 0);
            timeuse = 1000000 * (endapi.tv_sec - startapi.tv_sec) + endapi.tv_usec - startapi.tv_usec;
           //  timeuse /= 1000;
           //  if(timeuse >= 221000)//20%
           //      timeuse -= 221000;
            if(timeuse >= 124000)//40%
                timeuse -= 124000;
           //    if(timeuse >=158000)//30%
           //        timeuse -= 158000;
//            printf("%lf\n",timeuse);
//            printf("%lf\n",time_predict);
            if (timeuse > time_predict*2 && counter / ISSYNC != 1)//try
            {
//                printf("totally late\n");
               isLate = 1;
            }
        ...
        {
            if (sem_trywait(&pKernel_list_name[i]->sync.sch_sem)==0) //&& pKernel_list_name[i].sch.change_flag == 1)
            {
           //     printf("the process_flag is %d\n",pKernel_list_name[i]->sch.process_flag);
                // kkk[i]++;
                pKernel_list_name[i]->sch.num = i;
                (*need_schedule).emplace_back(pKernel_list_name[i]->sch);
                // printf("%d %d %d %d\n",kkk[0],kkk[1],kkk[2],kkk[3]);
                 scheduler[i] = 1;
            }
            if (sem_trywait(&pKernel_list_name[i]->sync.synced)==0)
            {

                GPU_resource += pKernel_list_name[i]->sch.GPU_ratio;
                max_bandwidth += pKernel_list_name[i]->sch.bandwidth;
                //sem_post(&pKernel_list_name[i]->sync.sem_finish);
                // int kernel_sem_synced_val;
                // sem_getvalue(&pKernel_list_name[i]->sync.synced,&kernel_sem_synced_val);
            //    printf("GPU_ratio get %d back from %d and GPU_resource is %d\n" ,pKernel_list_name[i]->sch.GPU_ratio, i,GPU_resource);
        ...
        for (int i = 0; i < (*kernel_schedule).size(); i++)
        {
           ...
           // }
			int tmpGPU = pKernel_list_name[Kernel_pos]->sch.GPU_ratio;
            GPU_resource -= pKernel_list_name[Kernel_pos]->sch.GPU_ratio;
            max_bandwidth -= pKernel_list_name[Kernel_pos]->sch.bandwidth;
             scheduler[Kernel_pos] = 0;
            sem_post(&pKernel_list_name[Kernel_pos]->sync.percent_sem[tmpGPU]);
            sem_trywait(&pKernel_list_name[Kernel_pos]->sync.sch_sem);
//            printf("process[%d] get %d GPU and GPU_resource is %d\n", Kernel_pos, pKernel_list_name[Kernel_pos]->sch.GPU_ratio,GPU_resource);
            //          }
        }
        ...
        for(int i=0;i<4;i++)
        {
            if(scheduler[i] == 1)
            {
                sem_post(&pKernel_list_name[i]->sync.sch_sem);
            }
        }
        need_schedule->clear();
    }
