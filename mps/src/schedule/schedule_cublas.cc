...
int main()
{
    ...
    ShmCublas *cublas_api = (ShmCublas *)mmap(NULL, sizeof(ShmCublas), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    cublas_api->init();
    assert(close(fd) != -1);
    ...
    while (true)
    {
        ...
        //scanf("%d",&a);
        //printf("using %d percentage\n",cublas_api->percent_flag);
        //if (cublas_api->process_flag == 0)
        if(sem_trywait(&cublas_api->sync.sch_sem)==0)
        {
            //printf("the process_flag is %d and changing to 1\n", cublas_api->process_flag);
            for (int i = 1; i <= 10; i++)
            {
                running_time[i] += cublas_api->cublas_call.duration[i];
            }
           // time_predict += cublas_api->cublas_call.duration[cublas_api->self_percent / 10];
            time_predict += cublas_api->cublas_call.duration[20 / 10];
           /* for (int i=1;i<=10;i++)
            {
                printf("%lf ", cublas_api->cublas_call.duration[i]);
            }
            printf("\n");*/
            counter++;
            if (counter % ISSYNC == 0)
            {
                sem_post(&cublas_api->sync.if_sync);
            }
            else if (counter % ISSYNC == 1)
            {
               // cublas_api->if_sync = 0;
                if (isLate == 1)
                {
                    // ......
                    int tmp_ratio = cublas_api->percent_flag;
                    for (int i = cublas_api->percent_flag; i <= 70; i += 10)
                    {
                        cublas_api->percent_flag = i;
                ...
                //    sem_post(&cublas_api->sync.percent_sem[cublas_api->percent_flag/10]);
                    printf("using %d percentage\n",cublas_api->percent_flag);
                    change = 1;
                    isLate = 0;
                }
                ...
                {
                    cublas_api->percent_flag = cublas_api->self_percent;
                    if(change == 1)
                    {
                        change = 0;
                        printf("roll back\n");
                ...
                GPU_resource -= cublas_api->percent_flag / 10;
                ss.push(cublas_api->percent_flag / 10);
                gettimeofday(&startapi, 0);

            }

            band_predict = cublas_api->cublas_call.bandwidth[cublas_api->percent_flag / 10];
            max_bandwidth -= band_predict;
            sum_band += band_predict;
           // cublas_api->process_flag = 1;
            if (cublas_api->self_percent == cublas_api->percent_flag)
            {
               // printf("cc\n");
                //sem_post(&cublas_api->sync.if_sync);
                sem_post(&cublas_api->sync.percent_sem[0]);
                pthread_barrier_wait(&cublas_api->sync.barrier); //不切换
                //sem_wait(&cublas_api->sync.synced);
                //pthread_barrier_wait(&cublas_api->sync.barrier);
               // printf("the process flag is %d\n", cublas_api->process_flag);
                // if(cublas_api->if_sync == 1){
                //     cublas_api->process_flag = 2;
                // }
                // else
                //     cublas_api->process_flag = -1;
               // printf("dd\n");
        ...
        if (sem_trywait(&cublas_api->sync.synced) == 0)
        {
            while(ss.size()>0)
            {
                GPU_resource += ss.top();
            ...
            //sem_post(&cublas_api->sync.sch_sem);
            gettimeofday(&endapi, 0);
            timeuse = 1000000 * (endapi.tv_sec - startapi.tv_sec) + endapi.tv_usec - startapi.tv_usec;
          //  timeuse /= 1000;
          //  if(timeuse >= 221000)//20%face
          //      timeuse -= 221000;
          //  if(timeuse >= 104000)//40%face
          //      timeuse -= 104000;
          //    if(timeuse >=158000)//30%face
          //        timeuse -= 158000;
             printf("%lf\n",timeuse);
             printf("%lf\n",time_predict);
          //  if (timeuse > time_predict*2 && counter / ISSYNC != 1)//face
            if(timeuse*1.7 > time_predict && counter / ISSYNC !=1 )//dig
            {
                isLate = 1;
            }
