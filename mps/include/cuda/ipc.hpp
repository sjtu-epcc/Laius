    ...
    void await_kernel()
    {
        ...
        while (true)
        {
            ...
            if (shared_memory->sch.GPU_ratio == percentage_flag)
            {
                ...
                else if (shared_memory->k_call.fptr == PATHFINDER_K_PTR)
                {
                    pathfinder_kernel = *(PathfinderKernel *)shared_memory->k_call.data;
                    args = pathfinder_kernel.to_args(shared_memory->memoryAllocations, pointers_ready, shared_memory->allocationIndex);
                }
                else
                {
                    printf("kernel parse not supported\n");
